...
for j = 1:3
    ...
    finetuneX = create_MNIST_CW_adversaries(train_x(1:5000,:), train_y(1:5000,:), control_nn);
    ...
    controlX = create_MNIST_CW_adversaries(test_x, test_y, control_nn);
    distilledX = create_MNIST_CW_adversaries(test_x, test_y, distillation_nn);
    finetuneX = create_MNIST_CW_adversaries(test_x, test_y, finetune_nn);
    sleepX = create_MNIST_CW_adversaries(test_x, test_y, sleep_nn);
    
    % Save networks
    save(strcat('CarliniWagner/', 'networks_', num2str(j)), 'control_nn', 'distillation_nn', 'finetune_nn', 'sleep_nn');
    
    % Save adversarial examples
    save(strcat('CarliniWagner/', 'adversaries_', num2str(j)), 'controlX', 'distilledX', 'finetuneX', 'sleepX');
end